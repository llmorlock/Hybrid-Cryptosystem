...
public:
    TripDES_(string key[], string imname, int mode, bool time_trial) {
        set_key(key);
        set_imname(imname);
        set_mode(mode);
    ...
    void set_key(string k[]) {
        if (k[0] == "d") {
            k[0] = "0123456789ABCDEF";
            k[1] = "0123456789ABCDEF";
            k[2] = "0123456789ABCDEF";
        }
        else if (k[0].length() != 16) {
    ...
    void DES_main() {
        ...
        // 3 color channels
        for (int c = 0; c < 3; c++) {
            cout << "3DES: Processing Channel " << c + 1 << "..." << endl;
            ...
            for (int k = 0; k < num_blocks; k++) {
                ...
                for (int y = 0; y < 3; y++) {
                    ...
                    hex_to_binary(key[y], binary_key);
                    ...
                    if (y == 0) {
                        ...
                        for (int i = 0; i < 64; i++) {
                            binary_input[i] = ciphertext_bin[i];
                        }
                    }
                    ...
                    if (mode == 2) {
                        if (k == 0) {
                            ...
                            bitwise_xor(CBC_storage[y], binary_input, 64);
                        }
                    }
                    ...
                    for (int i = 0; i < 16; i++) {
                        ...
                        // generate subkeys
                        pc_2_transform(mtx.PC_2, binary_key_56, subkeys[y], i);
                    }
                    ...
                    for (int i = 0; i < 16; i++) {
                        f_func(mtx.EXP, mtx.PERM, mtx.S_BOX, r_split, subkeys[y][i], s_perm);
                ...
                for (int y = 0; y < 3; y++) {
                    ...
                    key_reverse(subkeys[end], rev_keys);

                    if (y != 0) {
                        for (int i = 0; i < 64; i++) {
                            ciphertext_bin[i] = decoded_bin[i];
                        }
                    }
                    ...
                    if (mode == 2) {
                        if (k == 0) {
                            ...
                            bitwise_xor(CBC_storage[end], decoded_bin, 64);
                        }
                    }
                    ...
                    for (int i = 0; i < 64; i++) {
                        CBC_storage[end][i] = ciphertext_bin[i];
                    }
                    end--;
                }
            } // end of k for
